...
Headless mode ($ARGUMENTS contains `headless`): never prompt; everything a step would say or ask goes through `${CLAUDE_PLUGIN_ROOT}/scripts/notify.sh`.

1. Run, in order: `${CLAUDE_PLUGIN_ROOT}/scripts/freshen.sh` (report what it flags; headless: via notify.sh), `${CLAUDE_PLUGIN_ROOT}/scripts/task.sh sync`, `${CLAUDE_PLUGIN_ROOT}/scripts/inbound.sh`. Commit any uncommitted notes as written. Ignore `README.md`. No notes → ask the user what to build and stop (headless: notify "plan: nothing queued — text intent first" and stop).
...
4. Order by dependency, then present the list — feature grouping, each task's model pick, and for a task amending an open PR that PR's URL from the feature file — for approval (headless: skip; approval is the 🚀 message after the post in step 5). Decomposition quality is the leading indicator of pipeline success — spend your effort here.
