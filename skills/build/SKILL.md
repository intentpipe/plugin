...
4. Spawn the `reviewer` agent on the task — **always**, and always let it write a verdict to `review.md`, even on a resume where the branch is already complete and step 2 was a no-op. Its model follows task.md's `Review:` field: `light` → spawn with `model: sonnet`; `full`, unset or unknown → no override (the agent's own opus). A `light` review that returns the tier-mismatch finding (`[blocking] task.md — Review: light but the diff …`) is re-run once at full — spawn again without the override; that re-run is not a Round. A branch that already looks finished still flows review → done; never stop at Status `in-progress`. Then:
   - `VERDICT: approve` → step 5.
   - `VERDICT: blocking` and Rounds < 2 → increment Rounds in task.md, then spawn a NEW `implementer` agent (foreground, same `model`) with ONLY the blocking findings: "Fix review round <n> on task <id> — the branch already holds the implementation; fix only these, re-verify, commit: <findings>. Folder: …". Not a `SendMessage` to the finished one: a resumed agent re-uploads its whole context after idling (0072 paid 258k cache-write tokens for three resumes), and its completion reaches you only between tool calls — the orchestrator that polled `git log` in a 600s loop for it idled 18 minutes twice. A foreground Agent call is the wait. Then review again.
