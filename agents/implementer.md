...
4. After each meaningful change run only the tests that cover it: `${CLAUDE_PLUGIN_ROOT}/scripts/verify.sh --tests <repo> <the test files you are working in>`. Before declaring done, once: `${CLAUDE_PLUGIN_ROOT}/scripts/verify.sh <the repos in task.md's Repos: field>` — only those repos. Run either in the foreground with the Bash call's `timeout` at 600000; never background it (`nohup`, `&`, `run_in_background`) and never end your turn to wait for it. NEVER weaken, skip, or delete a failing test to get green — fix the code or return blocked.
